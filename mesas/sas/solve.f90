...
!
!        !$acc declare create(J_ts, Q_ts, weights_ts, SAS_lookup, P_list, C_J_ts, alpha_ts, k1_ts, C_eq_ts, C_old)
!        !$acc declare create(sT_init_ts, mT_init_ts, numcomponent_list, numbreakpt_list)
!        !$acc declare create(breakpt_index_list, component_index_list)
!        !$acc declare create(n_substeps, numflux, numsol, max_age)
!        !$acc declare create(timeseries_length, numcomponent_total, numbreakpt_total)
!        !$acc declare create(dt)
!        !$acc declare create(verbose, debug, warning)
...
!        end subroutine
!
!        subroutine update_aver(coeff, c)
!            ! Calculates the fluxes in the given the curent state
!            implicit none
!            !$acc routine
!            integer, intent(in) :: coeff, c
...
!        real(8) :: one8
!
!        !$acc declare copyin(dW_ts, C_Q_ts, sT_ts, mT_ts, ds_ts, dm_ts, dC_ts, dW_ts, pQ_ts, mQ_ts, mR_ts, WaterBalance_ts, SoluteBalance_ts)
!
